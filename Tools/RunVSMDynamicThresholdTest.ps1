...
$EvidenceRoot = Join-Path $ProjectRoot "Saved\Profiling\VSM_DynamicThreshold_20260816"
...
foreach ($Threshold in $Thresholds) {
    $Name = "DynamicThreshold$Threshold"
    ...
    $Arguments = @(
        ...
        "-BenchmarkAutoQuit",
        "-csvGpuStats",
        "-ExecCmds=`"r.Shadow.Virtual.CoarsePagePixelThresholdDynamic=$Threshold,stat unit,stat streaming`"",
        "-log=$LogName",
