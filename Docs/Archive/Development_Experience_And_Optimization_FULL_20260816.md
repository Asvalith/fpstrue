...
4. **查询过滤取舍**
   ...
   当前查询保留 `ECC_Pawn`，再按 `TargetCharacter` 和整轮命中集合过滤。本轮不增加专用近战通道，因为当前没有友伤、盾牌、可破坏物或多目标横扫规则，也没有数据证明 Pawn 候选是瓶颈。只有这些语义真实出现时再增加配置。

5. **墙体遮挡**

   当前武器查询只关心 Pawn。薄墙和门框必须纳入 PIE 回归；若确实出现隔墙命中，先在提交伤害前增加现有可见性/视线校验，再根据玩法复杂度决定是否升级查询规则。
...
需要区分历史问题和项目原实现：项目替换前使用的是 `FMath::VRandCone`。UE 官方将其定义为“在圆锥内均匀分布的随机单位向量”，它并不是错误 API。它追求的是圆锥立体角均匀；本次修改追求的是准星切平面或垂直靶面上的面积均匀。两者是不同的分布目标，小角度下差异很小。
...
这次修改主要调整“弹道分布目标”，不是 CPU 性能优化，也不能表述为修复了 `VRandCone` 的数学错误。

预期变化：

- 腰射落点在垂直靶面或准星圆范围内按面积均匀。
- 不会引入“半径线性随机”造成的中心异常密集。
- 霰弹多射线时，每颗弹丸覆盖圆盘面积更自然。
- 保持原有角度参数，避免重新调一整套武器数值。

当前还没有保存命中点热力图或固定随机种子 A/B 数据，因此不能写成“命中率提升 X%”，也不能把它包装成更真实的枪械弹道。更准确的表述是：弹道散布从立体角均匀圆锥采样改为切平面面积均匀采样，使落点分布更贴合圆形准星覆盖。

严格区分三种常见目标：

| 分布目标 | 数学含义 | 适用场景 |
| --- | --- | --- |
| 靶面面积均匀 | 当前实现，`r = sqrt(U) * tan(MaxAngle)` | 圆形准星覆盖、固定靶墙测试 |
| 圆锥立体角均匀 | `cos(theta)` 均匀，`VRandCone` 属于这一类 | 随机方向、球面概率解释 |
| 中心加权弹着群 | 二维高斯、截断高斯或设计曲线 | 更接近常见枪械弹着群或特定手感 |

当前项目散布角只有几度，前两种均匀分布的差异很小。后续面试可以讨论替换条件，但本轮不继续修改代码。
...
- 中心和边缘不应出现明显非预期密度偏差。
- 瞄准模式点云半径应明显小于腰射。
- 连续射击叠加散布时，点云半径应随 `ConsecutiveShotCount` 增大并受 `MaxContinuousFireSpreadAngle` 限制。
- 当前 `StopFire()` 会立即清零连续射击计数，因此普通松键路径是立即恢复；`SpreadResetDelay` 不是渐进恢复曲线。后续若被追问，可提出将当前扩散值按时间衰减，但它不属于本轮封版任务。
