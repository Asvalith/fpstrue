...
float AfpstrueEnemyAIController::GetNextDecisionInterval(const FDecisionContext& Context) const
{
	...
	const float SafeCombatResponseRangeMultiplier = FMath::IsFinite(CombatResponseRangeMultiplier)
		? FMath::Clamp(CombatResponseRangeMultiplier, 1.0f, 10.0f) : 1.5f;
	const bool bNeedsCombatResponse = ControlledEnemy->IsAttacking() || AIState == EFPEnemyAIState::Attack ||
		Context.DistanceSquared <= FMath::Square(ControlledEnemy->GetEffectiveAttackRange() * SafeCombatResponseRangeMultiplier);
...
void AfpstrueEnemyAIController::HandleSharedPursuit()
{
	...
	{
		const float SafePursuitAcceptanceRangeMultiplier = FMath::IsFinite(PursuitAcceptanceRangeMultiplier)
			? FMath::Clamp(PursuitAcceptanceRangeMultiplier, 0.0f, 1.0f) : 0.8f;
		const float PursuitAcceptanceRadius =
			FMath::Max(MoveAcceptanceRadius, ControlledEnemy->GetAttackRange() * SafePursuitAcceptanceRangeMultiplier);
